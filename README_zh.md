...
TraderHarness 就是这套规范：一个抗污染的 LLM 交易 Agent 回测执行环境——同时，由于每次模型调用都被全保真记录，它也是一个把这些运行一键变成 SFT 训练数据的数据合成器。
...
3. **SFT 导出。** `traderharness export sft` 输出 OpenAI 风格 JSONL，并由掩码与泄漏检查把关。
