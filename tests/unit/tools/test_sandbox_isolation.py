"""TDD tests for sandbox filesystem isolation (洞1).
...
Covers both registered ``run_python`` and ``execute_code`` sandboxes.
"""
...
class TestRunPythonFilesystemIsolation:
    @pytest.mark.asyncio
    ...
    async def test_cannot_import_os(self, tmp_path):
        ...
        ws.mkdir()
        ctx = _make_ctx(ws)
        result = await handle_run_python({"code": "import os\nresult = os.listdir('.')"}, ctx)
        assert "error" in result
    ...
    async def test_cannot_import_pathlib(self, tmp_path):
        ...
        ws.mkdir()
        ctx = _make_ctx(ws)
        result = await handle_run_python({"code": "import pathlib\nresult = 1"}, ctx)
        assert "error" in result
