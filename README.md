...
<p align="center">
  <strong>An execution and evidence environment for autonomous LLM trading agents — not another strategy library.</strong><br>
  Real A-share data · strict point-in-time masking · reproducible trajectories
</p>
...
TraderHarness is an execution environment and evaluation harness for LLM trading agents. The model receives a historically valid market, research tools, and a portfolio. It can investigate, write analysis code, revise its thesis, and place orders through one controlled path — nothing about the environment is a live market or a broker.

The first production dataset targets China A-shares: five years of full-market daily and 5-minute bars, announcements, policy news, fundamentals, valuation, dividends, and the CSI 300 benchmark.
...
Every claim above is meant to be checked, not taken on faith. Each completed run keeps, per LLM call: the full
message list, the full tool schema, assistant content and reasoning, every tool call and its arguments, and phase
and sub-window metadata — see [Training data](docs/training-data.md). The research console renders the same
evidence as a per-trade dossier: the executing 5-minute K-line, the agent's stated reasoning, and the exact tool
call that produced the fill.
...
If TraderHarness supports your research, please cite it:
